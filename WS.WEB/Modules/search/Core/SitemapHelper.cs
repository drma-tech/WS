...
namespace WS.WEB.Modules.Search.Core
{
        ...
        int maxDepth = 3)
    {
        ...
        private static string NormalizeUrlForVisit(string url)
        {
            try
            {
                var u = new Uri(url);
        ...
        private Dictionary<string, List<(string Href, string? Hreflang)>> BuildGroups(Dictionary<string, PageData> pagesByUrl)
        {
            ...
            foreach (var kv in groups.ToList())
            {
                ...
                var finalList = new List<(string Href, string? Hreflang)>();
                var seen = new HashSet<string>(StringComparer.OrdinalIgnoreCase);

                foreach (var v in list)
                {
                    var key = (v.Hreflang ?? "") + "|" + NormalizeHref(v.Href);
